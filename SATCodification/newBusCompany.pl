...
writeClauses:-  
    ...
    %%%news%%%
    departureFromPreviousDayArrival,
    eachTripMostOnePerDay,
    eachTripAtLeastMinimalPerWeek,
    busLessThanMaxKmTwoConsecutiveDays,
    true,!.
...
departureFromPreviousDayArrival:- bus(B), day(D1), day(D2), consecutiveDays(D1,D2), trip(C1-C2), trip(C3-C4), C2 \= C3, writeClause([-bdcc(B,D1,C1,C2), -bdcc(B,D2,C3,C4)]), fail.
departureFromPreviousDayArrival.

eachTripMostOnePerDay:- trip(C1-C2), day(D), findall( bdcc(B,D,C1,C2), bus(B), Lits), atMost(1,Lits), fail.
eachTripMostOnePerDay.

eachTripAtLeastMinimalPerWeek:- frequency(C1-C2,F), findall( bdcc(B,D,C1,C2), (bus(B), day(D)), Lits), atLeast(F, Lits), fail.
eachTripAtLeastMinimalPerWeek.

busLessThanMaxKmTwoConsecutiveDays:- bus(B), consecutiveDays(D1,D2), tooLongDist(C1-C2-C3), writeClause([-bdcc(B,D1,C1,C2), -bdcc(B,D2,C2,C3)]), fail.
busLessThanMaxKmTwoConsecutiveDays.
