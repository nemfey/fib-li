...
incompatibleWorkers(W1,W2):- worker(W1), worker(W2), maxScore(M), W1 \= W2,
                                workerScore(W1,S1), workerScore(W2,S2), ST is S1 + S2, ST >= M.  %% Complete this!
...
writeClauses:-  
    ...
    eachTeamAtMost,
    eachTeamAtLeast,
    %eachTeamAtMostAndAtLeast,
    workersIncompatiblesInDifferentTeams,
    true,!.
...
eachTeamAtMost:- team(T), maxSize(M), findall( wt(W,T), worker(W), Lits), atMost(M, Lits), fail.
eachTeamAtMost.

eachTeamAtLeast:- team(T), minSize(M), findall( wt(W,T), worker(W), Lits), atLeast(M, Lits), fail.
eachTeamAtLeast.

%eachTeamAtMostAndAtLeast:- team(T), maxSize(Max), minSize(Min), findall( wt(W,T), worker(W), Lits), atMost(Max, Lits), atLeast(Min, Lits), fail.
%eachTeamAtMostAndAtLeast.

workersIncompatiblesInDifferentTeams:- team(T), incompatibleWorkers(W1,W2), writeClause([-wt(W1,T), -wt(W2,T)]), fail.
workersIncompatiblesInDifferentTeams.
