...
eachTeamEachRoundOneMatch:- team(S), round(R), findall( match(S,T,R), otherTeam(S,T), LitsH ), findall( match(T,S,R), otherTeam(S,T), LitsA ), append(LitsH,LitsA,Lits), exactly(1,Lits), fail.
eachTeamEachRoundOneMatch.

eachOpponentExactlyOnce:- team(S), otherTeam(S,T), findall( match(S,T,R), round(R), LitsH ), findall( match(T,S,R), round(R), LitsA ), append(LitsH,LitsA,Lits), exactly(1,Lits), fail.
eachOpponentExactlyOnce.

homesAndAways:- team(S), otherTeam(S,T), round(R), writeClause([ -match(S,T,R),  home(S,R) ]), writeClause([ -match(S,T,R), -home(T,R) ]), fail.
homesAndAways.

noDoubles:- noDoubles(L), round(R), member(R,L), findall( double(S,R), team(S), Lits), exactly(0,Lits), fail.
noDoubles.
...
atMostOneDouble:- team(S), findall( double(S,R), round(R), Lits), atMost(1,Lits), fail.
atMostOneDouble.

doubleImplication:- round(R1), round(R2), R1 is R2-1, team(S), writeClause([-home(S,R1), -home(S,R2), double(S,R2)]), writeClause([home(S,R1), home(S,R2), double(S,R2)]), fail.
doubleImplication.
