...
satVariable( maxConsec(G,N) ):- gangster(G), hour(N).
...
writeClauses(MaxConsecutiveHours):-
    ...
    notGangsterAvailable,
    relateVars,
    checkMaxConsecHours(MaxConsecutiveHours),
    relateBusyWithMaxConsec(MaxConsecutiveHours),
    atLeastOneMaxConsec(MaxConsecutiveHours),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

gangstetrWorks:- needed(T,H,N), findall( does(G,T,H), available(G,H), Lits), atLeast(N,Lits), fail.
gangstetrWorks.

noTwoConsecTasks:- gangster(G), task(T1), task(T2), hour(H1), hour(H2), T1\=T2, H2 is H1+1, available(G,H1), available(G,H2),
...
notGangsterAvailable:- gangster(G), hour(H), blocked(G,H), writeClause([-busy(G,H)]), fail.
notGangsterAvailable.

relateVars:- available(G,H), findall( does(G,T,H), task(T), Lits), expressOr( busy(G,H), Lits), fail.
relateVars.

checkMaxConsecHours(MaxConsecutiveHours):- gangster(G), hour(H1), H2 is H1+MaxConsecutiveHours, hour(H2), findall( -busy(G,H), between(H1,H2,H), Lits),
                                            writeClause(Lits), fail.
checkMaxConsecHours(_).

relateBusyWithMaxConsec(MaxConsecutiveHours):- gangster(G), hour(H1), hour(H2), H2 > H1, Diff1 is H2 - H1 + 1, Diff2 is Diff1 + 0, Diff2 > MaxConsecutiveHours,
                                                findall(busy(G,H), (hour(H), H2 >= H, H >= H1), Lits), expressAnd( maxConsec(G,Diff1), Lits), fail.
relateBusyWithMaxConsec(_).

atLeastOneMaxConsec(MaxConsecutiveHours):- findall(maxConsec(G,H), (gangster(G), hour(H)), Lits), atLeast(MaxConsecutiveHours,Lits), fail.
atLeastOneMaxConsec(_).
...
costOfThisSolution(M,Cost):- findall( maxConsec(G,H), member(maxConsec(G,H), M), Lits), max2(Lits,Cost), !.

max2([],0).
max2([maxConsec(_,H)], H).
max2([maxConsec(_,H) | Xs], R):- max2(Xs,T), (H>T -> R=H; R=T).
