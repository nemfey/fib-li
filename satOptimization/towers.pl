...
writeClauses(MaxNumTowers):-
    ...
    significantVillagesWithTower,
    eachVillageWatchedByAtLeastOneTower,
    noTowersOutsideVillage,
    relateTowerPosWithTowerVillage,
...
%significantVillagesWithTower:- significantVillage(V), writeClause([towerVillage(V)]), fail.
significantVillagesWithTower:- significantVillage(V), findall(towerPos(I,J), posVillage(V,I,J), Lits), atLeast(1,Lits), fail.
significantVillagesWithTower.

eachVillageWatchedByAtLeastOneTower:- village(V), findall( towerPos(I,J), posWatchesVillage(V,I,J), Lits), atLeast(1,Lits), fail.
eachVillageWatchedByAtLeastOneTower.

%noTowersOutsideVillage:- position(I,J), findall(towerPos(I,J), not(posVillage(_,I,J)), Lits), exactly(0,Lits), fail.
noTowersOutsideVillage:- position(I,J), not(posVillage(_,I,J)), writeClause([-towerPos(I,J)]), fail.
noTowersOutsideVillage.

relateTowerPosWithTowerVillage:- village(V), findall( towerPos(I,J), posVillage(V,I,J), Lits), expressOr(towerVillage(V),Lits), fail.
relateTowerPosWithTowerVillage.

atMostMaxTowers(MaxNumTowers):- findall(towerVillage(V), village(V), Lits), atMost(MaxNumTowers,Lits), fail.
atMostMaxTowers(_).
